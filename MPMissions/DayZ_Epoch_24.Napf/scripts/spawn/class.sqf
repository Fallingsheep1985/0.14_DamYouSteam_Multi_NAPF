private ["_binocs","_currentSex","_dragee","_hasBinocs","_hasPistol","_hasPistolAmmo","_itemNew","_model","_myModel","_pistols","_pistolAmmo","_primaryWep","_qty"];
...
if (!_isPZombie) then {
	...
	{if (_x in _binocs) then {_hasBinocs = true;};} count (classSelect select 3);
